function Natoms = write_graphene_wall_file(Ny, r, Ltotal, path)
...
%add half d to radius
% h = sqrt(3/4)*d;
h=d/3;
ind = [sqrt(X(1,:).^2 + X(2,:).^2) <= r+h;sqrt(X(3,:).^2 + X(4,:).^2) <= r+h];
...
Natoms = Ny*(Ny-1)*2*2; %first *2 for two atoms in unit cell, second *2 because two walls
...
for n = 1:size(ind, 2)
    ...
    if all(ind(:,n))
        ...
        Natoms = Natoms - 2*2; %because there will be two walls later on
    else
        if ind(1,n)==1
            %cut C1
